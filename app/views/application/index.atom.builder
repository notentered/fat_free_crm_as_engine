...
atom_feed do |feed|
  ...
  assets.each do |asset|
    feed.entry(asset) do |entry|
      entry.title   !asset.is_a?(FatFreeCRM.user_class) ? asset.name : "#{asset.full_name} (#{asset.username})"
      entry.summary send(:"#{item}_summary", asset) if respond_to?(:"#{item}_summary")

      entry.author do |author|
        author.name !asset.is_a?(FatFreeCRM.user_class) ? asset.try(:user).try(:full_name) : asset.full_name
      end
