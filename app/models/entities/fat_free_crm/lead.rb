...
class FatFreeCrm::Lead < ActiveRecord::Base
  ...
  def save_with_permissions(params)
    ...
    if params[:lead][:access] == "Campaign" && self.campaign # Copy campaign permissions.
      save_with_model_permissions(FatFreeCrm::Campaign.find(self.campaign_id))
    else
  ...
  def promote(params)
    ...
    opportunity = FatFreeCrm::Opportunity.create_for(self, account, params[:opportunity])
...
private
  ...
  def increment_leads_count
    if self.campaign_id
      FatFreeCrm::Campaign.increment_counter(:leads_count, self.campaign_id)
    end
  end
  ...
  def decrement_leads_count
    if self.campaign_id
      FatFreeCrm::Campaign.decrement_counter(:leads_count, self.campaign_id)
    end
  end
