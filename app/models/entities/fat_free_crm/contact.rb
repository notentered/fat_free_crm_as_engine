...
class FatFreeCrm::Contact < ActiveRecord::Base
  ...
  def save_with_account_and_permissions(params)
    save_account(params)
    result = self.save
    self.opportunities << FatFreeCrm::Opportunity.find(params[:opportunity]) unless params[:opportunity].blank?
    result
  end
  ...
  def save_account(params)
    if params[:account][:id] == "" || params[:account][:name] == ""
      ...
      if self.account != account and account.id.present?
        self.account_contact = FatFreeCrm::AccountContact.new(:account => account, :contact => self)
      end
    end
