...
class FatFreeCrm::User < ActiveRecord::Base
  ...
  #----------------------------------------------------------------------------
  def awaits_approval?
    self.suspended? && self.login_count == 0 && FatFreeCrm::Setting.user_signup == :needs_approval
  end
  ...
  def deliver_password_reset_instructions!
    ...
    FatFreeCrm::UserMailer.password_reset_instructions(self).deliver
  end
  ...
  def check_if_needs_approval
    self.suspended_at = Time.now if FatFreeCrm::Setting.user_signup == :needs_approval && !self.admin
  end

  # Prevent current user from deleting herself.
  #----------------------------------------------------------------------------
  def check_if_current_user
    FatFreeCrm::User.current_user.nil? || FatFreeCrm::User.current_user != self
  end
  ...
  class << self
    ...
    def current_ability
      FatFreeCrm::Ability.new(FatFreeCrm::User.current_user)
    end
