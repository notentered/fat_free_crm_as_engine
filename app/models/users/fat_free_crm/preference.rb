...
class FatFreeCrm::Preference < ActiveRecord::Base
  ...
  def [] (name)
    ...
    cached_prefs[name.to_s] = if (self.user.present? && pref = FatFreeCrm::Preference.find_by_name_and_user_id(name.to_s, self.user.id))
  ...
  def []= (name, value)
    ...
    if pref = FatFreeCrm::Preference.find_by_name_and_user_id(name.to_s, self.user.id)
      pref.update_attribute(:value, encoded)
    else
      FatFreeCrm::Preference.create(:user => self.user, :name => name.to_s, :value => encoded)
    end
